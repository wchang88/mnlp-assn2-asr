...
if [ -z "${QUECHUA}" ]; then
    log "Fill the value of 'QUECHUA' of db.sh"
    exit 1
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
   ...
   python3 local/data_prep.py ${QUECHUA} sph2pipe
   # Uncomment the line below and comment the line above to switch to multilingual data prep
   #  python local/data_prep_multilingual.py ${GUARANI} sph2pipe
